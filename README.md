...
VPBuddy 是一个 AI 会议协同与交付生成系统。当前仓库包含同一套 UI 的网页端和 Electron 桌面壳；后端服务、Hermes、ASR、知识库和 AI 能力全部通过既有 API 调用，不内置到前端或安装包。

工作台现已接入后端行业模板库。登录后可在“行业模板”中搜索、按行业筛选、查看鉴权预览，并将模板应用为一场新会议；成功后会直接进入该会议的交付物页查看后端生成的 Demo V1。模板、封面、预览和应用结果均来自后端，不使用前端示例数据。

## 网页端

网页端与桌面客户端共用页面、样式、状态管理和接口客户端，不另建一套 UI。Web 服务通过同源代理透明转发现有 HTTP API、文件上传下载、SSE 和实时 ASR WebSocket，因此接口路径、字段和凭证方式保持不变。
...
网页端和桌面客户端都只加载前端 UI。会议、材料、知识库、AI 协同、交付物等能力均通过同一套后端 API 获取或提交。
...
`npm run dev` 与 `npm run web` 都会启动带后端同源代理的网页服务。
