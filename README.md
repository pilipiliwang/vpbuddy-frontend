...
- [macOS Apple Silicon 版（arm64）](https://github.com/pilipiliwang/vpbuddy-frontend/releases/download/v0.1.3/VPBuddy-0.1.3-mac-arm64.dmg)：适用于 Apple M 系列芯片。
- [macOS Intel 版（x64）](https://github.com/pilipiliwang/vpbuddy-frontend/releases/download/v0.1.3/VPBuddy-0.1.3-mac-x64.dmg)：适用于 Intel 芯片 Mac。

每个正式版本只上传上述 4 个带版本号的用户安装文件。GitHub 另行自动提供源码 zip 和 tar.gz。

## v0.1.3 更新摘要

- 投屏区改为稳定的统一渲染画布，PDF 可滚动阅读且不再被轮询刷新重置；图片、PDF 和其他可预览材料均可批注和截屏。
- 截屏始终捕获投屏画布区域，并与手动发送材料共用上传进度、会议资料刷新和对话记录流程。
- 发送材料入口不再因页面刷新或 Tab 切换失效；上传成功、后端解析中、解析完成或失败分别反馈。
- 交付物页左侧可在会议记录与交付物列表之间切换，录制按钮在原位置强化状态和可点击提示。
- AI 协同列表与详情兼容后端字段差异，统一按安全 Markdown 层级展示。
- 打包客户端在页面加载完成、加载失败或超时场景均可可靠显示窗口，修复安装后进程存在但界面不出现的问题。
