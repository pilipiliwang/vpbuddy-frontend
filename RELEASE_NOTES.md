...
- 当前本地测试地址：`http://127.0.0.1:4180/`。该地址仅在本机 VPBuddy Web 服务运行时可访问，其他设备无法通过此地址连接。
- 本地启动：先执行 `npm ci`，再通过 PowerShell 执行 `$env:PORT="4180"; npm run web`。
- 未指定 `PORT` 时，网页端默认使用 `http://127.0.0.1:4173/`。
- 网页端与桌面客户端复用同一套 UI、接口路径、请求字段和登录凭证；当前不提供独立安装包。
- 公网 HTTPS 域名部署完成后，将在此处补充正式网页地址。

### 桌面客户端

- 需要本地安装和独立窗口体验的用户，请继续下载下方对应版本的 Windows 或 macOS 安装包。
- 网页端预览不会替换、删除或停止已有桌面客户端；现有 `v0.1.4` 安装包和历史版本继续保留。
- Windows 提供安装版和便携版，macOS 分别提供 Apple Silicon 与 Intel 版本，请按设备架构选择。
